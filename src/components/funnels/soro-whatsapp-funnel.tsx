...
const DynamicHeader: React.FC = () => {
  ...
  const functionalitiesItems = (
     <>
        ...
        <DropdownMenuSeparator />
        <DropdownMenuLabel className="text-muted-foreground font-semibold text-xs">FUNCIONALIDADES</DropdownMenuLabel>
  ...
  const resourcesItems = (
    <>
        ...
        <Link href="/demo"><DropdownMenuItem className="gap-3"><PlayCircle size={20}/> Realizar Demo</DropdownMenuItem></Link>
        <Link href="/lead-magnet"><DropdownMenuItem className="gap-3"><BookOpen size={20}/> Ver Ebook</DropdownMenuItem></Link>
        <Link href="/webinars"><DropdownMenuItem className="gap-3"><Presentation size={20}/> Ver Webinars</DropdownMenuItem></Link>
        <Link href="/vsl-opt-in"><DropdownMenuItem className="gap-3"><Video size={20}/> Ver Video de Ventas</DropdownMenuItem></Link>
        <Link href={whatsappLink} target="_blank" onClick={() => handleTrackEvent('Contact')}><DropdownMenuItem className="gap-3"><WhatsappIcon className="h-5 w-5"/> Contactar por WhatsApp</DropdownMenuItem></Link>
    </>
  );

  const navItems = [
    { name: 'Funcionalidades', isDropdown: true, content: <div className="grid grid-cols-2 gap-4 p-2">{functionalitiesItems}</div>, mobileContent: functionalitiesItems },
    { name: '¿Para quién es?', isDropdown: true, content: <div className="p-2">{forWhomItems}</div>, mobileContent: forWhomItems },
    { name: 'Recursos', isDropdown: true, content: <div className="p-2">{resourcesItems}</div>, mobileContent: resourcesItems },
  ];

  return (
    <header 
      className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMenuOpen ? 'bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 shadow-md py-2' : 'bg-transparent py-4'
      }`}
    >
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div 
            ...
            className="md:hidden bg-black/60 fixed inset-0 z-40"
          ...
          >
            <motion.nav 
              ...
              className="bg-white absolute top-0 right-0 h-full w-full max-w-sm p-6 pt-24 flex flex-col"
            ...
            >
              <Accordion type="single" collapsible className="w-full">
                {navItems.map((item) => (
                  <AccordionItem value={item.name} key={item.name}>
                    {item.isDropdown ? (
                      <>
                        <AccordionTrigger className="text-lg font-semibold py-4">{item.name}</AccordionTrigger>
                        <AccordionContent>
                      ...
                      <Link href="#" className="flex w-full items-center py-4 text-lg font-semibold border-b">
                        {item.name}
                      </Link>
                    )}
                  </AccordionItem>
              ...
              <div className="mt-auto pt-8">
                <Button onClick={handleDemoClick} className="bg-teal-500 hover:bg-teal-600 text-white w-full text-lg py-6">
                  Ver Demo
                </Button>
              </div>
            </motion.nav>
...
const WhySoroSection: React.FC = () => {
  ...
  return (
    <SectionWrapper className="py-12 md:py-16 bg-white">
      <div className="container mx-auto px-4">
